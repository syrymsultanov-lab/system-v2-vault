...
"""Patch WF TG Conversation Inbound (EEMvbCJaiN8affDR):
  Add RANK_QUALIFICATION_FORMAT style rule to 80_Build_Prompt systemPrompt:
    - All rank/qualification answers MUST be in package counts, not %
    - Format: «X premium ИЛИ Y classic ИЛИ Z starter ИЛИ mix»
    - Always activation + recurring sections
    - Per-leg cap with concrete numbers
  Plus 2 few-shot examples (MD, SMD).

Builds on patch_inbound_compliance_hard_rules.py — preserves 104RU compliance,
adds package-format layer.

Idempotent — overwrites jsCode with same template each run.
"""
import json
...
- Если в КБ нет точных чисел для запрошенного ранга — НЕ считай сам, верни escalate_reason='no_kb'.
...
def patch():
    ...
    target["parameters"]["jsCode"] = BUILD_PROMPT_CODE

    out_path = ROOT / ".tmp" / "80_build_prompt_format_packages.js"
    out_path.parent.mkdir(exist_ok=True)
    out_path.write_text(BUILD_PROMPT_CODE, encoding="utf-8")
    ...
    assert "Compliance Hard Rules (104RU" in deployed_code, "patch did not apply: 104RU compliance markers missing"
    ...
    assert "Формат ответа на квалификацию ранга" in deployed_code, "patch did not apply: package format rule missing"
    assert "ВСЕГДА отвечай **конкретными числами пакетов**" in deployed_code, "patch did not apply: package format rule body missing"
    assert "ОБЯЗАТЕЛЬНО разделяй 3 пути в ответе" in deployed_code, "patch did not apply: лично/командно/mix split missing"
    assert "НИКОГДА не упоминай слова «процент»" in deployed_code, "patch did not apply: no-% rule missing"
    assert "Few-shot examples" in deployed_code, "patch did not apply: few-shot examples missing"
    assert "ЛИЧНО (сам активируешь Membership)" in deployed_code, "patch did not apply: ЛИЧНО marker missing in few-shot"
    assert "ЧЕРЕЗ КОМАНДУ (3 ветки партнёров" in deployed_code, "patch did not apply: ЧЕРЕЗ КОМАНДУ marker missing in few-shot"
    assert "MIX (лично + команда)" in deployed_code, "patch did not apply: MIX marker missing in few-shot"
    assert "НЕ отвечай на квалификацию рангов процентами" in deployed_code, "patch did not apply: anti-percent antipattern missing"

    print("80_Build_Prompt updated with RANK_QUALIFICATION_FORMAT rules + few-shot.")
    print(f"Saved local copy: {out_path}")
    print("[OK] verified: deployed jsCode contains 104RU compliance + package-format rules + MD/SMD few-shot")
